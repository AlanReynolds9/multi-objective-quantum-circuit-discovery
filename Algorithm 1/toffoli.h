...
// This is going to be almost identical to fourier.h. We should perhaps work out how to avoid this code duplication.
...
namespace toffoli
{
  ...
  {
  public:
    using Problem = toffoli::Problem;  // Required by the algorithm solution classes such as NSGA2Solution.
    ...
    // that is not dominated (and that we might wish to be stored) should not be beaten either. I.e. A beats B => A
    // dominates B. These will usually be the same, but we may wish to use a modified dominance relation within the
  ...
  private:
    ...
    void evaluate_error() override;  // The hard bit - required circuit simulations.
  ...
  bool Solution::dominates(const Solution& rhs) const  // Inlined, as this gives a significant speed boost. (Clang)
  {
    ...
    if (overallError() < problem_.targetError())
    {
      ...
      // (We could, instead, have objective functions that correct the error to zero (or even -1.0, given that I've seen
      // negative error values crop up). However, at present, I would also want the objective functions to return the
      // 'calculated' error values too, just for interest's sake.)
  ...
  bool Solution::beats(const Solution& rhs) const  // Inlined, as this gives a significant speed boost. (Clang)
  {
