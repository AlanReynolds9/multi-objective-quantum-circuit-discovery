...
namespace toffoli
{
  ...
  public:
    ...
    double overallError() const;  // Access solution quality information by name, rather than by using objective(x).
  ...
  bool Solution::dominates(const Solution& rhs) const  // Inlined, as this gives a significant speed boost. (Clang)
  {
    ...
    {                // TWO OBJECTIVES: Comment out third objective.
      return false;
    }
    return total_cost < rhs.total_cost || overallError() < rhs.overallError() - tolerance ||
           worstCaseError() < rhs.worstCaseError() - tolerance;  // TWO OBJECTIVES: Comment out third objective.
  }
