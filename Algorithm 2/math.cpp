...
size_t binCoeff(size_t n, size_t r)
{
  ...
  // Was used in hashing - hence the 'size_t'. Feel free to switch to int or long.
...
size_t combinationRank(const vector<int>& combination)  // Currently unused. Feel free to change size_t to int or long.
{
...
vector<vector<pair<int, cmplx> > > sparse(const vector<vector<cmplx> >& matrix)
{
  int size = static_cast<int>(matrix.size());
  vector<vector<pair<int, cmplx> > > sparseMatrix(size);
  for (int row = 0; row < size; ++row)
  {
    for (int col = 0; col < size; ++col)
    {
      auto& element = matrix[row][col];
      if (abs(element) > constants::tolerance)
      {
        sparseMatrix[row].push_back(make_pair(col, element));
      }
    }
  }
