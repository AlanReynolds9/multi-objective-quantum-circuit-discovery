...
public:
  ...
  std::vector<int> qbits() const;  // Output controls as a sorted vector of ints. (Logically, it is a set.) Slow.
  bool isControl(int qbit) const;
  int first() const;  // First control qbit. If there are none, return num_qbits.

  // Output the controls as a list (or rather, a vector) of qbit indices.
  std::vector<int> asList() const;
