...
namespace fourier
{
                   ...
                   badErrorCutoff, gateOptProb, numInsertions, minInsertionLength, maxInsertionLength, angleTolerance)
  {
    ...
    // the Fourier problem. (If we were to add auxiliary qbits, it would be a different matter.)
  }
...
//    return 3;  // For three objectives...
  ...
  double Problem::objUpperBound(int objNum) const
  {
    ...
    {
      return 1.0;  // Maximum value for either error measure.
    }
  }
  ...
  {
    // The target state is the Fourier transform of the start state.
    return start_state(sim).fourier();
  }
  ...
  {
    // Overlaps for the various possible inputs have been calculated. We just need to use these to construct the correct
    // error values.
  ...
  void Solution::calculate_symbolic()
  {
    ...
    //
    // In the comments, * means conjugation, not multiplication - we will use x for multiplication.
    ...
    for (int j = 0; j < 3; ++j)
    {
      ...
      overlapTotal[j] /= num_simulations();  // Unnecessary, but we leave as is, to make the comment below correct
    }
    ...
    primary_error_coeffs[0][0] = -norm(overlapTotal[0]);  // Constant part, which is -a*a. (* being conjugation!)
